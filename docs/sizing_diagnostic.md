...
## Post-fix outcome — seed 42, preset quality, after equalize_body_zones_pass2
    ...
    x_heights (30 eligible): [11, 12, 3, 12, 12, 11, 13, 11, 12, 12, 13,
                              12, 12, 7, 13, 9, 12, 12, 11, 11, 12, 9, 12,
                              12, 9, 10, 12, 9, 8, 10]
    min x_h = 3 (it)
    max x_h = 13 (on)
    x_height_spread (max/min) = 4.333

### Outcome

- spread 5.500 -> 4.333 (21% reduction, beats the 15% bar).
- The max-side outliers (`so` 22, `was` 21, `on` 20) were scaled down
  to ~13 by `equalize_body_zones_pass2`. Ink heights for those words
  dropped from 24-25 to 14-22 px, which reads as "short words are
  shorter than tall words" -- more natural.
- `I` also scaled down (`_effective_x_height` returns ink_h for
  single-char tokens whose x-height is noise; pass2 caught "I" the
  same way it caught the short no-ascender words).
- The min-side outliers (`it` 3, `by` 7) did not improve. Rescuing
  those requires inflating their ink heights past the `height_outlier`
  ceiling, which the regression gate rejects. 1.4 target is therefore
  structurally unreachable on this pipeline; 4.333 is the practical
  floor at the x-height-spread layer.
- Regression on seeds 42/137/2718: `height_outlier_score` and
  `ocr_min` primary gates hold on all three.
- test-quick: all 302 pass (+ 4 new cases for `equalize_body_zones_pass2`).
